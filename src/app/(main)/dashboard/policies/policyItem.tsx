    ...
    <div className='mb-2 border-slate-400 border-solid border rounded-lg px-6 py-4 grid grid-cols-3'>
      <div className='col-span-2'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 mt-4'>
          <div className='mb-1'>
            ...
            <h4 className="text-alternGray font-semibold">{new Date(policyItem?.startDate).getDay()} de {new Date(policyItem?.startDate).toLocaleString('es', {month: 'long'})} del {new Date(policyItem?.startDate).getFullYear()}</h4>
          </div>

          <div className='mb-1'>
            <h4 className="text-alternGray">Fin de vigencia</h4>
            <h4 className="text-alternGray font-semibold">??</h4>
          </div>
      ...
      <div className='flex flex-col mx-auto'>
        <div className='border-slate-400 border-solid border rounded-lg px-4 py-2'>
          <p className='text-lg text-lightBlue font-semibold text-center'>??</p>
        </div>
        <Button
          className="text-center flex mt-auto mb-2 min-h-12 bg-darkBlue" onClick={() => router.push(`/dashboard/policies/${policyItem?.serialNumber}`)}>
          Ver más
        </Button>
      </div>
    </div>
